...
- **Holding the entry.** `computeManifestEntries` offers a dependency only once the sweep that counted it has its counts delivered. An unloaded dependency has no delta, and that is correct: a counting sweep found zero classes, and the confirmed flush after it proves no count is in flight. A dependency's entry therefore goes out at least one flush after its counts.
- **Holding a mapping.** An external class that maps to a dependency whose entry has not been delivered is held until it has. A mapping never names a dependency id the collector has not seen. An absent reference names no dependency, so it is never held for this reason.
- **The flag.** `ProbeManifest.dependencies_listed` is true on a manifest when three things hold as it is built. The listing is complete. Every dependency whose discovery source is the startup classpath has been delivered. Every external class recorded before the listing completed has been delivered or resolved to nothing to send. From then on every manifest carries true, like `references_recorded`. When the flag first becomes true and the flush has no other manifest to send, the agent sends an empty manifest to carry it. A listing that failed never sets it.
...
- A dependency's entry and the flag arrive one flush later than before. The listing itself runs as ADR 0030 describes.
