...
ByteBuddy refuses to redefine a class carrying an annotation whose `@Target` does not include `TYPE`. Kotlin's `@file:JvmName` is the common case: kotlinc attaches it to the class file even though its target list does not allow that. Yukon's type matcher excludes such a class before ByteBuddy commits to rebasing it, and records the skip with a reason. `TransformFailureListener` catches any other transform failure, rolls back the speculative registration, and records the skip the same way. Skipped classes travel to the collector in the manifest's `skipped_classes` list. "Not instrumented" is an honest state to report; "in the manifest, permanently zero" is not.
